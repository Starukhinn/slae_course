...
vector<AuxiliaryCoefficients> FindAuxiliaryCoefficients(const ThreeDiagonalMatrix &matrix,
                                                        const NumbersColumn &free_members) {
    ...
    for (auto number_line = 0; number_line < matrix.GetSize() - 1; ++number_line) {
        ...
        double q_coef;
        if (number_line == 0) {
            p_coef = -1 * matrix.GiveElement(0, 1) / matrix.GiveElement(0, 0);
            q_coef = free_members.GiveElement(0) / matrix.GiveElement(0, 0);
        } else {
            ...
            q_coef = (free_members.GiveElement(number_line) -
                      matrix.GiveElement(number_line, 0) *
...
NumbersColumn Progonka(const ThreeDiagonalMatrix &matrix, const NumbersColumn &free_members) {
    ...
    vector<AuxiliaryCoefficients> array_of_auxiliary_coefficients =
        FindAuxiliaryCoefficients(matrix, free_members);
    for (int number_line = matrix.GetSize() - 1; number_line >= 0; --number_line) {
        ...
        if (number_line == matrix.GetSize() - 1) {
            ...
            answer_member = array_of_auxiliary_coefficients[number_line].p_coef *
                                unknown_members.GiveElement(number_line + 1) +
